...
use dasl::drisl::{DecodeError, Value, de, de::from_slice, error::Len, to_vec};
use serde::{Deserialize, Serialize};
use serde_tuple::{Deserialize_tuple, Serialize_tuple};

#[test]
fn test_string1() {
    let drisl: Result<Value, _> = de::from_slice(&[0x66, 0x66, 0x6f, 0x6f, 0x62, 0x61, 0x72]);
    assert_eq!(drisl.unwrap(), Value::Text("foobar".to_string()));
}

#[test]
fn test_string2() {
    let drisl: Result<Value, _> = de::from_slice(&[
        0x71, 0x49, 0x20, 0x6d, 0x65, 0x74, 0x20, 0x61, 0x20, 0x74, 0x72, 0x61, 0x76, 0x65, 0x6c,
        0x6c, 0x65, 0x72,
    ]);
    assert_eq!(drisl.unwrap(), Value::Text("I met a traveller".to_string()));
}

#[test]
fn test_string3() {
    let slice = b"\x78\x2fI met a traveller from an antique land who said";
    let drisl: Result<Value, _> = de::from_slice(slice);
    assert_eq!(
        drisl.unwrap(),
        Value::Text("I met a traveller from an antique land who said".to_string())
    );
}

#[test]
fn test_byte_string() {
    let drisl: Result<Value, _> = de::from_slice(&[0x46, 0x66, 0x6f, 0x6f, 0x62, 0x61, 0x72]);
    assert_eq!(drisl.unwrap(), Value::Bytes(b"foobar".to_vec()));
}

#[test]
fn test_numbers1() {
    let drisl: Result<Value, _> = de::from_slice(&[0x00]);
    assert_eq!(drisl.unwrap(), Value::Integer(0));
}

#[test]
fn test_numbers2() {
    let drisl: Result<Value, _> = de::from_slice(&[0x1a, 0x00, 0xbc, 0x61, 0x4e]);
    assert_eq!(drisl.unwrap(), Value::Integer(12345678));
}

#[test]
fn test_numbers3() {
    let drisl: Result<Value, _> = de::from_slice(&[0x39, 0x07, 0xde]);
    assert_eq!(drisl.unwrap(), Value::Integer(-2015));
}
...
fn test_numbers_large_negative() {
    ...
    assert_eq!(drisl.unwrap(), Value::Integer(expected));
}

#[test]
fn test_bool() {
    let drisl: Result<Value, _> = de::from_slice(b"\xf4");
    assert_eq!(drisl.unwrap(), Value::Bool(false));
}

#[test]
fn test_trailing_bytes() {
    let drisl: Result<Value, _> = de::from_slice(b"\xf4trailing");
    assert!(matches!(drisl.unwrap_err(), DecodeError::TrailingData));
}

#[test]
fn test_list1() {
    let drisl: Result<Value, _> = de::from_slice(b"\x83\x01\x02\x03");
    assert_eq!(
        drisl.unwrap(),
        Value::Array(vec![
            Value::Integer(1),
...
#[test]
fn test_list2() {
    let drisl: Result<Value, _> = de::from_slice(b"\x82\x01\x82\x02\x81\x03");
    assert_eq!(
        drisl.unwrap(),
        Value::Array(vec![
            Value::Integer(1),
...
#[test]
fn test_object() {
    let drisl: Result<Value, _> = de::from_slice(b"\xa5aaaAabaBacaCadaDaeaE");
    let mut object = BTreeMap::new();
    object.insert("a".to_string(), Value::Text("A".to_string()));
    object.insert("b".to_string(), Value::Text("B".to_string()));
    object.insert("c".to_string(), Value::Text("C".to_string()));
    object.insert("d".to_string(), Value::Text("D".to_string()));
    object.insert("e".to_string(), Value::Text("E".to_string()));
    assert_eq!(drisl.unwrap(), Value::Map(object));
}

#[test]
fn test_indefinite_object_error() {
    let drisl: Result<Value, _> = de::from_slice(b"\xbfaa\x01ab\x9f\x02\x03\xff\xff");
    let mut object = BTreeMap::new();
    object.insert("a".to_string(), Value::Integer(1));
    object.insert(
        "b".to_string(),
        Value::Array(vec![Value::Integer(2), Value::Integer(3)]),
    );
    assert!(matches!(drisl.unwrap_err(), DecodeError::IndefiniteSize));
}

#[test]
fn test_indefinite_list_error() {
    let drisl: Result<Value, _> = de::from_slice(b"\x9f\x01\x02\x03\xff");
    assert!(matches!(drisl.unwrap_err(), DecodeError::IndefiniteSize));
}
...
fn test_indefinite_byte_string_error() {
    let drisl: Result<Value, _> = de::from_slice(b"\x5f\x42\x01\x23\x42\x45\x67\xff");
    assert!(matches!(drisl.unwrap_err(), DecodeError::IndefiniteSize));
}
...
fn test_multiple_indefinite_strings_error() {
    ...
    let drisl: Result<Value, _> = de::from_slice(input);
    assert!(matches!(drisl.unwrap_err(), DecodeError::IndefiniteSize));
}
...
fn test_crazy_list() {
    ...
    let drisl: Vec<Value> = de::from_slice(slice).unwrap();
    assert_eq!(
        drisl,
        vec![
...
#[test]
fn test_nan() {
    let drisl: Result<f64, _> = de::from_slice(b"\xf9\x7e\x00");
    assert!(matches!(drisl.unwrap_err(), DecodeError::Mismatch { .. }));
}
...
fn truncated_object() {
    ...
    .concat();
    dasl::drisl::from_slice::<Value>(&input).expect_err("truncated");
}
...
fn test_default_values() {
    let basic_cases = [
        ...
        TestCase {
            ...
            expected: Expected::Err(
                |err| matches!(err, DecodeError::RequireLength { name, found } if *name == "TupleWithDefaultsStruct" && matches!(found, Len::Small(3))),
            ),
        },
    ];
    ...
    let tuple_overflow_cases = [
        ...
        TestCase {
            hex: "828301020304",
            expected: Expected::Err(
                |err| matches!(err, DecodeError::RequireLength{ name, found} if *name == "TupleIntInner" && matches!(found, Len::Small(3))),
            ),
        },
        ...
        T: serde::de::DeserializeOwned + std::fmt::Debug + PartialEq,
    {
        for case in test_cases {
            let input = hex::decode(case.hex).unwrap();
